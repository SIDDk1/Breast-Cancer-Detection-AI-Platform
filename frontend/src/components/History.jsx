    ...
    <div className={`history-item ${expanded ? 'expanded' : ''}`} id={`history-item-${index}`}>
      <div className="history-item-main" onClick={() => setExpanded(!expanded)}>
        ...
        <div className="history-thumb-wrap">
          {item.overlay_image || item.input_image ? (
            <img
              src={item.overlay_image || item.input_image}
              alt="result"
          ...
          <div className="history-images">
            {item.input_image && (
              <div className="history-img-wrap">
                <div className="history-img-label">Original</div>
                <img src={item.input_image} alt="original" className="history-img"
                  onError={(e) => { e.target.style.display = 'none'; }}/>
              </div>
            )}
            {item.mask_image && (
              <div className="history-img-wrap">
                <div className="history-img-label">Mask</div>
                <img src={item.mask_image} alt="mask" className="history-img"
                  onError={(e) => { e.target.style.display = 'none'; }}/>
              </div>
            )}
            {item.overlay_image && (
              <div className="history-img-wrap">
                <div className="history-img-label">Overlay</div>
                <img src={item.overlay_image} alt="overlay" className="history-img"
                  onError={(e) => { e.target.style.display = 'none'; }}/>
              </div>
            )}
          </div>
